...
export default function AddListingPage() {
  ...
  return (
    <main className="min-h-screen min-w-0 bg-[#f8f9fa] py-16 sm:py-20">
      <div className="mx-auto max-w-4xl min-w-0 px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <button
            type="submit"
            disabled={submitting}
            className="mt-8 flex min-h-12 h-12 w-full items-center justify-center gap-2 rounded-md text-sm font-semibold text-white shadow-sm transition-all duration-200 hover:shadow-md active:scale-[0.99] focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-70"
            style={{
...
const inputClass =
  "block h-12 w-full min-w-0 rounded-md border border-gray-300 bg-white px-4 text-base text-zinc-900 placeholder:text-zinc-400 outline-none transition-all duration-200 focus:border-[#2ed2c3] focus:ring-2 focus:ring-[#2ed2c3]/30 md:text-sm";
        ...
        <div className="absolute left-0 right-0 top-[calc(100%+6px)] z-30 overflow-hidden rounded-md border border-zinc-200 bg-white shadow-2xl">
          <div className="flex items-center gap-2 border-b border-zinc-100 px-3 py-2.5">
            ...
            <input
              ...
              className="min-w-0 flex-1 border-0 bg-transparent py-1 text-base text-zinc-900 outline-none placeholder:text-zinc-400 md:text-sm"
            />
            {query && (
              <button
                type="button"
...
function ImageUpload({ preview, fileName, onChange, onClear }) {
  return (
    <div>
      {preview ? (
        <div className="flex min-w-0 flex-wrap items-center gap-4 rounded-md border border-zinc-200 bg-zinc-50 p-3 sm:flex-nowrap">
          {/* eslint-disable-next-line @next/next/no-img-element -- user-selected preview (data URL or remote); next/image not applicable */}
          <img
            src={preview}
            alt="Preview"
...
function ThankYouModal({ open, name, onClose }) {
  useEffect(() => {
    if (!open) return;
    function onKey(e) {
