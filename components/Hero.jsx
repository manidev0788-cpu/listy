...
export function Hero() {
  ...
  const handleSearch = (e) => {
    ...
    const cat = catRaw === "All Categories" ? "" : catRaw;
  ...
  return (
    ...
    >
      <div className="absolute inset-0" aria-hidden suppressHydrationWarning>
        {slides.map((slide, i) => (
          <div
            key={slide.src}
            suppressHydrationWarning
          ...
          <div className="flex flex-col rounded-[28px] bg-white p-2 shadow-[0_12px_40px_rgba(0,0,0,0.25)] ring-1 ring-black/5 md:flex-row md:items-center md:rounded-full">
            <div className="flex min-h-[52px] flex-1 items-center gap-3 border-b border-zinc-200 px-4 py-2 md:border-b-0 md:border-r md:px-5">
              ...
              <input
                ...
                className="min-w-0 flex-1 border-0 bg-transparent py-1 text-left text-base text-zinc-900 outline-none placeholder:text-zinc-400 md:text-sm"
              />
            </div>
            ...
            <div className="mt-1 md:mt-0 md:shrink-0 md:pl-2">
              <button
                type="submit"
                className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-linear-to-br from-[#2ed2c3] to-[#1fa99c] px-7 text-sm font-semibold text-white transition-opacity hover:opacity-90 active:scale-[0.99] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2ed2c3] focus-visible:ring-offset-2 md:h-11 md:min-h-0 md:w-auto md:min-w-[140px]"
              >
                <SearchIcon />
                Search
              </button>
