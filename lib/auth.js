...
/** Demo defaults only outside production — production admins must set ADMIN_EMAIL or ADMIN_EMAILS. */
const DEFAULT_ADMIN_EMAILS = isProd ? [] : ["admin@gmail.com"];
...
async function upsertOAuthUser({ email, name, image, provider }) {
  ...
  try {
    ...
    if (!existing) {
      ...
      const authCurrent = Array.isArray(existing.authProviders)
        ...
        await users.updateOne(
          { _id: existing._id },
          {
            $set: {
              ...
              authProviders: authCurrent.includes(provider)
                ? authCurrent
                : [...authCurrent, provider],
              updatedAt: new Date(),
            },
          }
        );
...
export const authOptions = {
  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      const storedEmail = await upsertOAuthUser({
        email: user.email,
        name: user.name,
        image: user.image,
